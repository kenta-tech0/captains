...
以下 3 つをインストールしてください。（Mac / Windows どちらも OK）
...
3. **Dev Containers 拡張機能**（VS Code 内でインストール）
...
docker system prune -a --volumes       # Docker 全体のゴミ掃除（注意：他プロジェクトにも影響）
```

そのあと Dev Container を Rebuild してください。
